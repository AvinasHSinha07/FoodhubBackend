...
router.patch('/users/:id/status', auth('ADMIN'), validateRequest(UserValidation.updateUserStatusZodSchema), UserController.updateUserStatus);
// Order Monitoring (Admin Only)
router.get('/orders', auth('ADMIN'), OrderController.getMyOrders);
// Coupon Management (Admin Only)
router.post('/coupons', auth('ADMIN'), validateRequest(CouponValidation.createCouponZodSchema), CouponController.createCoupon);
router.get('/coupons', auth('ADMIN'), CouponController.getAllCoupons);
router.patch('/coupons/:id', auth('ADMIN'), validateRequest(CouponValidation.updateCouponZodSchema), CouponController.updateCoupon);
...
router.post('/categories', auth('ADMIN'), validateRequest(CategoryValidation.createCategoryZodSchema), CategoryController.createCategory);
router.put('/categories/:id', auth('ADMIN'), validateRequest(CategoryValidation.updateCategoryZodSchema), CategoryController.updateCategory);

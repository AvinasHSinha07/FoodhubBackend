...
const getAdminOverviewAnalytics = async (days) => {
    ...
    const orderStatusBreakdownMap = new Map();
    orders.forEach((order) => {
    ...
    return {
        summary: {
            ...
            paidOrders: orders.filter((order) => order.paymentStatus === PaymentStatus.PAID ||
                order.paymentStatus === PaymentStatus.COD_COLLECTED).length,
            pendingPayments: orders.filter((order) => order.paymentStatus === PaymentStatus.PENDING ||
                order.paymentStatus === PaymentStatus.COD_PENDING).length,
        },
...
const getProviderOverviewAnalytics = async (providerUserId, days) => {
